...
13*. SELECT k.nazwa AS klasa, k.wychowawca, COUNT(p.id_ucznia) AS uczniowie_z_projektem, COUNT(u.id_ucznia) - COUNT(p.id_ucznia) AS uczniowie_bez_projektu FROM klasy k JOIN uczniowie u USING(id_klasy) LEFT JOIN projekty p USING(id_ucznia) GROUP BY k.id_klasy, k.wychowawca;
