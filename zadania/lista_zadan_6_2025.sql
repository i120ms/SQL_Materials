...
FROM continents ct
JOIN regions r ON r.continent_id = ct.continent_id
...
(
    ...
    WHERE r2.continent_id = ct.continent_id
)
ORDER BY ct.name, c.area DESC;
...
8.
SELECT COUNT(DISTINCT country_id) AS liczba_krajow
FROM country_stats
...
11.
SELECT COUNT(*) AS liczba_regionow
FROM (
    SELECT region_id
    FROM countries
    GROUP BY region_id
